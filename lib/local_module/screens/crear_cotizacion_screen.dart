...
class _CrearCotizacionScreenState extends State<CrearCotizacionScreen> {
  ...
  Future<void> _mostrarSelectorUnidadMedida() async {
    final val = await SelectUnidadMedidaModal.show(context, seleccionInicial: _seleccionUnidadMedida);
    if (val != null && mounted) {
      setState(() {
  ...
  Widget _buildSummary() {
    return TotalSummaryCard(
      subtotal: _subtotalOriginal,
      ahorro: _ahorroTotalItems,
      descuento: _montoDescuento,
      total: _total,
      moneda: 'Bs',
    );
