...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    // Si la plataforma de ejecución es Web o Windows Desktop, mostramos el nuevo Dashboard Sidebar.
    // Si es una aplicación móvil nativa (Android / iOS / etc.), mostramos la interfaz clásica de Mobile.
    if (kIsWeb || defaultTargetPlatform == TargetPlatform.windows) {
      // Si estamos en Web pero en un navegador móvil (pantalla menor a 768 de ancho),
      // mostramos la interfaz clásica móvil que es responsive y cómoda.
      if (kIsWeb && MediaQuery.of(context).size.width < 768) {
        return _buildMobileLayout(context);
      }
      return _buildWebDashboardLayout(context);
    } else {
      return _buildMobileLayout(context);
    }
  }

  // --- INTERFAZ WEB / ESCRITORIO (Sidebar Dashboard) ---
  Widget _buildWebDashboardLayout(BuildContext context) {
    ...
    final pendingProducts = _allProducts.where((p) => p.estado.trim().toLowerCase() == 'pendiente').toList();
    final listos = _allProducts.where((p) => p.estado.trim().toLowerCase() == 'listo').toList();

    List<Map<String, dynamic>> filteredQuotes = _cotizaciones;
    if (_documentFilter == 'sin_nota') {
      filteredQuotes = _cotizaciones.where((c) => !_tieneNotaEntrega(c)).toList();
    } else if (_documentFilter == 'con_nota') {
      filteredQuotes = _cotizaciones.where((c) => _tieneNotaEntrega(c)).toList();
    }

    if (_searchQuery.isNotEmpty && _activeTab == 'cotizaciones') {
      final query = _searchQuery.toLowerCase();
      filteredQuotes = filteredQuotes.where((c) {
        final cliente = (c['clienteNombre'] ?? '').toString().toLowerCase();
        final id = c['id'].toString();
        return cliente.contains(query) || id.contains(query);
      }).toList();
    }
            ...
            child: ListView(
              ...
              children: [
                _sidebarTile(
                  ...
                  label: "Inicio",
                  active: _activeTab == 'inicio',
                  onTap: () => setState(() => _activeTab = 'inicio'),
                ),
                _sidebarTile(
                  icon: Icons.calculate_outlined,
                  label: "Cotizaciones",
                  active: _activeTab == 'cotizaciones',
                  onTap: () => setState(() => _activeTab = 'cotizaciones'),
                ),
                _sidebarTile(
                  icon: Icons.point_of_sale,
                  label: "Punto de Venta",
                  active: _activeTab == 'venta_institucional',
                  onTap: () => setState(() => _activeTab = 'venta_institucional'),
                ),
                _sidebarTile(
                  icon: Icons.inventory_2_outlined,
                  label: "Inventario",
                  active: _activeTab == 'inventario',
                  onTap: () => setState(() => _activeTab = 'inventario'),
                ),
                if (session.isAdmin) ...[
                  _sidebarTile(
                    icon: Icons.people_alt_outlined,
                    label: "Clientes",
                    active: _activeTab == 'clientes',
                    onTap: () => setState(() => _activeTab = 'clientes'),
                  ),
                  _sidebarTile(
                    icon: Icons.manage_accounts_outlined,
                    label: "Personal",
                    active: _activeTab == 'personal',
                ...
                _sidebarTile(
                  ...
                  active: false,
                  onTap: () => UpdateService.buscarActualizacion(context),
                ),
              ],
            ),
          ),
            ...
            child: Column(
              children: [
                SwitchListTile(
                  ...
                  title: const Text("Modo Oscuro", style: TextStyle(color: Colors.white70, fontSize: 13)),
                  secondary: Icon(isDark ? Icons.dark_mode : Icons.light_mode, color: primaryCyan, size: 20),
                  value: isDark,
                ...
                ListTile(
                  ...
                  title: const Text("Cerrar Sesión", style: TextStyle(color: Colors.white70, fontSize: 13)),
                  onTap: () async {
                    Session().clear();
                    _drive.signOut();
                    final prefs = await SharedPreferences.getInstance();
    ...
    // Cabecera superior
    Widget topHeaderWidget = Container(
      height: 70,
        ...
        children: [
          ...
          if (size.width >= 600) ...[
            IconButton(
              ...
              tooltip: "Google Debug",
              onPressed: _showGoogleDebugDialog,
            ),
            IconButton(
              icon: const Icon(Icons.calendar_today_rounded, color: Colors.grey, size: 20),
              onPressed: () {},
            ),
          ],
          Stack(
            children: [
              IconButton(
                icon: const Icon(Icons.notifications_outlined, color: Colors.grey, size: 22),
                onPressed: () {},
              ),
              Positioned(
                right: 12,
          ...
          PopupMenuButton<String>(
            ...
            child: Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    (session.userName ?? 'U').substring(0, 1).toUpperCase(),
                      ...
                      Text(
                        session.role == UserRole.developer
                            ...
                            : (session.role == UserRole.admin 
                                ? "Administrador" 
                                : (session.role == UserRole.designer ? "Diseñador" : "Vendedor")),
            ...
            itemBuilder: (context) => [
              PopupMenuItem(
                value: 'profile',
                child: Text("Hola, ${session.userName}"),
              ),
              const PopupMenuItem(
                value: 'logout',
    ...
    Widget activeContentWidget;
    switch (_activeTab) {
      case 'inicio':
        if (session.role == UserRole.admin || session.role == UserRole.developer) {
          activeContentWidget = SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 6),
                Text(
                  "Bienvenido al panel de control de Novaled.",
                  style: TextStyle(
                    fontSize: 14,
          ...
          activeContentWidget = SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 6),
                Text(
                  "Bienvenido al panel de control de Novaled. Selecciona una sección en el menú lateral para comenzar o utiliza los accesos rápidos:",
                  style: TextStyle(
                    fontSize: 14,
                    color: isDark ? Colors.white60 : Colors.black54,
                  ),
              ...
              GridView(
                ...
                children: [
                  _buildStatCard(
                    title: "Cotizaciones",
                    value: _cotizaciones.length.toString(),
                  ...
                  _buildStatCard(
                    title: "Clientes",
                    value: _clientesList.length.toString(),
              ...
              Wrap(
                ...
                children: [
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => CrearCotizacionScreen(
                            displayId: _cotizaciones.length + 1,
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text("Nueva Nota de Entrega"),
                    style: ElevatedButton.styleFrom(
                  ...
                  ElevatedButton.icon(
                    ...
                    label: const Text("Subir Producto / Foto"),
                    style: ElevatedButton.styleFrom(
                    ...
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(builder: (context) => const AddArtScreen(isCatalog: true)),
                      ).then((_) => _loadAll());
                    },
                  ),
                ],
          ...
          child: Column(
            ...
            children: [
                      ...
                      children: [
                        Expanded(
                          child: ElevatedButton.icon(
                            ...
                            label: const Text("Cotización", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 12)),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: primaryCyan,
                        ...
                        Expanded(
                          child: ElevatedButton.icon(
                            ...
                            label: const Text("Nota de Entrega", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 12)),
                            style: ElevatedButton.styleFrom(
                              backgroundColor: Colors.tealAccent[400]!,
                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                              padding: const EdgeInsets.symmetric(vertical: 12),
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ElevatedButton.icon(
                          icon: const Icon(Icons.add, size: 16, color: Colors.black),
                          label: const Text("Cotización", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: primaryCyan,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                          ),
                        ...
                        ElevatedButton.icon(
                          icon: const Icon(Icons.add_shopping_cart, size: 16, color: Colors.black),
                          label: const Text("Nota de Entrega", style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
                          style: ElevatedButton.styleFrom(
                            backgroundColor: Colors.tealAccent[400]!,
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                          ),
              ...
              Wrap(
                ...
                runSpacing: 8,
                children: [
                  _documentFilterButton(label: "Todas", value: 'todas'),
                  _documentFilterButton(label: "Sin Nota de Entrega", value: 'sin_nota'),
                  _documentFilterButton(label: "Con Nota de Entrega", value: 'con_nota'),
                ],
              ),
              const SizedBox(height: 20),
              Expanded(
                child: Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: filteredQuotes.isEmpty
                        ? const Center(child: Text("No se encontraron cotizaciones.", style: TextStyle(color: Colors.grey)))
                        : size.width < 700
                            ? _buildMobileQuoteList(filteredQuotes, isDark, primaryCyan)
                                ...
                                TableRow(
                                  ...
                                  children: const [
                                    Padding(
                                      ...
                                      child: Text("ID", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.grey)),
                                    ),
                                    Padding(
                                      padding: EdgeInsets.symmetric(horizontal: 16.0, vertical: 15.0),
                                      child: Text("Cliente", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.grey)),
                                    ),
                                    Padding(
                                      padding: EdgeInsets.symmetric(horizontal: 16.0, vertical: 15.0),
                                      child: Text("Fecha", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.grey)),
                                    ),
                                    Padding(
                                      padding: EdgeInsets.symmetric(horizontal: 16.0, vertical: 15.0),
                                      child: Text("Total (Bs)", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.grey)),
                                    ),
                                    Padding(
                                      padding: EdgeInsets.symmetric(horizontal: 16.0, vertical: 15.0),
                                      child: Text("Documento", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: Colors.grey)),
                                    ),
                                  ],
                                ),
                                for (var index = 0; index < filteredQuotes.length; index++)
                                  _buildTableRow(filteredQuotes[index], index, isDark, primaryCyan),
                              ],
                            ),
                          ),
                  ),
      ...
      case 'venta_institucional':
        activeContentWidget = VentaInstitucionalScreen(tipoVenta: 'venta_institucional');
        break;

      case 'notas_entrega':
        activeContentWidget = const NotasEntregaScreen(hideAppBar: true);
        break;

      case 'catalogo':
        activeContentWidget = local_inv.InventoryScreen(
          hideAppBar: true, 
      ...
      case 'foto_a_arte':
        activeContentWidget = InventoryScreen(
          products: pendingProducts,
          onRefresh: _loadAll,
          title: null,
          filter: (p) => p.estado.trim().toLowerCase() == 'pendiente',
          preferRawThumbnail: true,
          autoRefresh: true,
        );
        break;

      case 'clientes':
        activeContentWidget = ClientsScreen(hideAppBar: true);
        break;

      case 'almacenamiento_configuracion':
        activeContentWidget = AlmacenamientoConfiguracionScreen(hideAppBar: true);
        break;

      case 'almacenamiento_demicachos':
        activeContentWidget = AlmacenamientoScreen(hideAppBar: true);
        break;

      case 'personal':
        activeContentWidget = PersonalManageScreen(hideAppBar: true);
        break;

      default:
        activeContentWidget = const Center(child: Text("Pestaña no encontrada"));
    }
      ...
      pageBuilder: (context, anim1, anim2) {
        return StatefulBuilder(
          builder: (context, setPopupState) {
            ...
            final textCol = isDarkLocal ? Colors.white : const Color(0xFF0F172A);
            final dialogBgLocal = isDarkLocal ? const Color(0xFF1E293B) : Colors.white;

            return Align(
              alignment: Alignment.topRight,
              child: Container(
                margin: const EdgeInsets.only(top: 105, right: 16),
                ...
                child: Material(
                  ...
                  child: Column(
                    ...
                    children: [
                      SwitchListTile(
                        title: Text("Modo Oscuro", style: TextStyle(color: textCol, fontSize: 15, fontWeight: FontWeight.w600)),
                        value: isDarkLocal,
                        activeColor: const Color(0xFF00ADEF),
                        onChanged: (val) {
                          NovaledApp.of(context).toggleTheme(val);
                      ...
                      Theme(
                        ...
                        child: ExpansionTile(
                          ...
                          title: Text("Configuración", style: TextStyle(color: textCol, fontSize: 15, fontWeight: FontWeight.w600)),
                          collapsedIconColor: textCol,
                          iconColor: textCol,
                          children: [
                            if (session.isAdmin)
                              ListTile(
                                contentPadding: const EdgeInsets.only(left: 32),
                                title: Text("Personal", style: TextStyle(color: textCol, fontSize: 14)),
                                onTap: () {
                                  Navigator.pop(context);
                                  Navigator.pushNamed(context, '/personal').then((_) => _loadAll());
                                },
                              ),
                            if (session.isAdmin)
                              ListTile(
                                contentPadding: const EdgeInsets.only(left: 32),
                                title: Text("Personalización", style: TextStyle(color: textCol, fontSize: 14)),
                                onTap: () {
                                  Navigator.pop(context);
                            ...
                            ListTile(
                              contentPadding: const EdgeInsets.only(left: 32),
                              title: Text("Buscar Actualización", style: TextStyle(color: textCol, fontSize: 14)),
                              onTap: () {
                                Navigator.pop(context);
                      ...
                      ListTile(
                        title: const Text("Cerrar Sesión", style: TextStyle(color: Colors.redAccent, fontSize: 15, fontWeight: FontWeight.w600)),
                        onTap: () async {
                          Navigator.pop(context);
                          Session().clear();
                          _drive.signOut();
