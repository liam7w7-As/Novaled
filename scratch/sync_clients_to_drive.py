...
def drive_create_or_update_data_json(access_token, folder_id, client_data):
    ...
    json_bytes = json.dumps(client_data, ensure_ascii=False).encode("utf-8")
...
def sync_single_client(access_token, clientes_folder_id, client_dict, idx, total_count):
    client_id = client_dict["id"]
    name = client_dict["nombreCompania"].strip()
    clean_name = name.lower().strip()
    folder_id = client_dict.get("folderId")
    ...
    try:
        ...
        # 2. If not in cache and not set locally, create a folder on Drive (network call outside lock)
        if not folder_id:
            print(f"[{idx}/{total_count}] Creating folder on Drive for: {name}...")
            res_folder = drive_create_folder(access_token, name, clientes_folder_id)
            folder_id = res_folder["id"]
            # Thread-safe cache update
            with cache_lock:
                drive_cache[clean_name] = folder_id
                
        client_dict["folderId"] = folder_id
        
        # 3. Upload or update data.json inside client's folder
        print(f"[{idx}/{total_count}] Syncing data.json for: {name}...")
        drive_create_or_update_data_json(access_token, folder_id, client_dict)
        
        return client_id, folder_id
    except Exception as e:
        print(f"[{idx}/{total_count}] Error syncing {name}: {e}")
...
def main():
    try:
        ...
        # 2. Get or create CLIENTES
        print("Checking CLIENTES folder...")
        clientes_folders = drive_list_files(access_token, f"'{db_root_id}' in parents and name='CLIENTES' and mimeType='application/vnd.google-apps.folder' and trashed=false")
        if clientes_folders:
            clientes_folder_id = clientes_folders[0]["id"]
        else:
            print("Creating CLIENTES folder...")
            clientes_folder_id = drive_create_folder(access_token, "CLIENTES", db_root_id)["id"]
            
        # 3. Retrieve all existing client folders under CLIENTES on Drive to build a cache
        print("Caching existing folders from Google Drive...")
        existing_drive_folders = drive_list_files(access_token, f"'{clientes_folder_id}' in parents and mimeType='application/vnd.google-apps.folder' and trashed=false")
        ...
        local_clients = [dict(c) for c in cursor.fetchall()]
        print(f"Found {len(local_clients)} clients in SQLite database.")
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = {
                executor.submit(sync_single_client, access_token, clientes_folder_id, client, idx, len(local_clients)): client
                for idx, client in enumerate(local_clients, 1)
            }
            
            for future in as_completed(futures):
                client_id, folder_id = future.result()
                if folder_id:
                    results.append((folder_id, client_id))
        ...
        cursor.executemany("UPDATE clientes SET folderId = ? WHERE id = ?", results)
        conn.commit()
        conn.close()
        
        print("\n--- Parallel Sync Summary ---")
        print(f"Total synchronized clients: {len(results)}")
        print("Google Drive parallel synchronization completed successfully.")
        
    except Exception as e:
        print(f"Error in sync_clients_to_drive: {e}")
        sys.exit(1)
